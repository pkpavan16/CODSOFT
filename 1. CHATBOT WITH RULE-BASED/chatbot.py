...
from utils import current_timestamp, fuzzy_score, load_json, normalize_text, safe_eval, save_json, today_key
...
class IntelligentChatbot:
    ...
    def _calculate_math(self, message):
        # Clean the message of math query words to extract the expression
        clean_expr = re.sub(
            r"\b(calculate|solve|what is|whats|math|equal to|equals|value of)\b",
            "",
            message,
            flags=re.IGNORECASE
        )
        # Find expression like character sequences (numbers, operators, parentheses)
        math_match = re.search(r"([0-9+\-*/().\s]+)", clean_expr)
        if not math_match:
            return "a basic expression like (5 + 3) * 2"

        expr_str = math_match.group(1).strip()
        # Clean leading/trailing symbols that are not numbers/parentheses
        ...
        try:
            ...
            if isinstance(result, float) and result.is_integer():
                return int(result)
            return int(result) if isinstance(result, int) else round(result, 4)
        except ZeroDivisionError:
            return "undefined because division by zero is not allowed"
        except Exception:
            return f"a valid expression (could not parse '{expr_str}')"
