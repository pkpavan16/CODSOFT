...
def test_safe_eval_math_direct():
    from utils import safe_eval
    assert safe_eval("(5 + 3) * 2") == 16
    assert safe_eval("100 / 4 - 5") == 20
    assert safe_eval("-5.5 + 10.5") == 5.0


def test_advanced_math_intent():
    bot = IntelligentChatbot()
    result = bot.respond("calculate (12 - 4) * 3")
    assert "24" in result["response"]
